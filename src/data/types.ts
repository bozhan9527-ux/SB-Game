...
export interface PowerBalance {
   ...
   * 競技場的山門耐久要放大幾倍。
   *
   * **它其實是「操作時間預算」的旋鈕。** 競技場把所有養成歸零，所以能不能
   * 守住完全看你多快把符合起來——而實測顯示那是一道懸崖，不是斜坡：
   * 每 2.2 秒動一次手的人穩穩打到 7～8 波，每 3 秒動一次手的人有六成
   * 連第 1 波都過不了，每 4 秒的人是百分之百。手機上拖一張符本來就要
   * 兩三秒，所以真人正好落在懸崖上。
   *
   * 而「連第 1 波都過不了」的下場不是分數低，是**什麼都沒有**——
   * 伺服器會退回 0 波的紀錄（「一波都還沒下」），他連榜上一列都拿不到。
   *
   * 2 倍是掃出來的：3 秒的人從六成過不了變成中位 6 波，4 秒的人至少拿得到
   * 1 波，而 1～2 秒的人還是 7～8 波——天花板沒動，差距還在。
   * 3 倍就過頭了：全部落在 6～8 波，技術不再分得出高下，
   * 那正是這個榜最初要避免的事。
   */
  arenaDiscipleMultiplier: number;
}
