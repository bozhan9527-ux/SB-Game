...
**唯一的例外是遙測**（`src/telemetry/`，2026-08-29 加入）。它不是後端：
用的是現成的服務（PostHog），沒有自己的伺服器、沒有帳號、沒有存檔上傳，
只送五個不含個資的事件。之所以是例外，是因為「玩家在第幾關流失」這種問題
不靠資料就永遠只能猜，而它擋住的是後面每一個營運與平衡決定。
沒有設定金鑰時整包是 no-op，也就是說預設的 build 仍然是完全不連網的單機版。
